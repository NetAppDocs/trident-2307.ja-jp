...
ブロックサイズと帯域幅は、IOPS数に直接的な影響を与えます。ブロックサイズが大きくなるにつれて、システムはより大きなブロックサイズを処理するために必要なレベルまで帯域幅を増加させます。帯域幅が増加すると、システムが達成できるIOPS数は減少します。QoSとパフォーマンスの詳細については、 https://www.netapp.com/pdf.html?item=/media/10502-tr-4644.pdf["SolidFire のサービス品質"^]を参照してください。
