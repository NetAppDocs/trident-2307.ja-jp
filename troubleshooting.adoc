...
* Tridentを https://azure.microsoft.com/en-us/products/netapp/["Azure NetApp Files の特長"]と併用する際に発生する可能性のある一般的な問題は、テナントとクライアントのシークレットが、権限が不十分なアプリ登録から取得される場合です。Tridentの要件の完全なリストについては、link:trident-use/anf.html["Azure NetApp Files の特長"]の設定を参照してください。
